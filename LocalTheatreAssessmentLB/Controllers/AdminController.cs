...
namespace LocalTheatreAssessmentLB.Controllers
{
    public class AdminController : Controller
    {
            ...
            try
            {
                ...
                if (UsersInRole > 0)
                {
                    throw new Exception(String.Format(
                        "Canot delete {0} Role because it still has uesrs.", RoleName));
                }
                ...
                {
                    throw new Exception(String.Format(
                        "Canot delete {0} Role does not exists.", RoleName));
                }
                List<RoleDTO> colRoleDTO = (from objRole in roleManager.Roles
                                            select new RoleDTO
                                            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        #region public ActionResult EditRoles(UserAndRoleDTO paramUserAndRoleDTO)
        ...
        private void DeleteUser(ExpandedUserDTO paramExpandedUserDTO)
        {
            ...
            //if we could not find the user, thow an exception
            if (user == null)
            {
                throw new Exception("Could not find the User");
            }
        ...
        private List<string> RolesUserIsNotIn(string UserName)
        {
            ...
            //go get the roles for and individual
            ApplicationUser user = UserManager.FindByName(UserName);
            //if we could noit find the user, throw an exception
            if (user == null)
            {
                throw new Exception("Could not find the user");
            }
